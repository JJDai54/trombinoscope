 ...
 * @return string
 */
function trombinoscope_block_addCatSelect($cats)
{
    $cat_sql = '(';
    if (\is_array($cats)) {
...
/**
 * Get the number of events from the sub categories of a category or sub topics of or topic
 * @param $mytree
 * @param $events
 * @param $entries
 * @param $cid
 * @return int
 */
function trombinoscopeNumbersOfEntries($mytree, $events, $entries, $cid)
{
    $count = 0;
    if(\in_array($cid, $events)) {
        $child = $mytree->getAllChild($cid);
        foreach (\array_keys($entries) as $i) {
            if ($entries[$i]->getVar('evt_id') == $cid){
                $count++;
            }
            foreach (\array_keys($child) as $j) {
                if ($entries[$i]->getVar('evt_id') == $j){
                    $count++;
                }
            }
        }
...
function trombinoscopeMetaKeywords($content)
{
...
function trombinoscopeMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
 ...
 */
function trombinoscope_RewriteUrl($module, $array, $type = 'content')
{
    $comment = '';
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = trombinoscope_Filter(xoops_getModuleOption('static_name', $module));
    }
 ...
 * @return string $url
 */
function trombinoscope_Filter($url, $type = '') {
